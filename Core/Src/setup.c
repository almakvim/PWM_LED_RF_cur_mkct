...
//	SegAddr = FLASH_PR_BASE;
	...
	if( Setup.CSum != CRC16( &Setup, sizeof( Setup ) - sizeof( Setup.CSum ) ))
	{
		...
		Setup.SerialId = 4200;            
		Setup.bt_coef = 0.01f;
		Setup.pwm_coef = 1.001f;
		Setup.start_level = 50;
		Setup.delta = 25;
        Setup.coef_curr1 = -0.00464f;
        Setup.coef_curr2 = 14.552f;
        Setup.shift_curr = 0.0f;
        Setup.limit_curr = 0.45f;

	}
	dev_var.count_pkt_in = 0;
	dev_var.count_pkt_out = 0;
    dev_var.quant = Setup.limit_curr/100.0f;
}
